...
InlineContentData = namedtuple("InlineContentData", ["type", "image", "html_attributes"])
...
def measure_time(func):
    ...
    def measure_time_wrapper(*args, **kwargs):
        ...
        logger.debug(f"{msg_prefix_start}Calling {func.__name__}() with args {args} and kwargs {kwargs}")
        ...
        logger.debug(f"{msg_prefix_end}Call {func.__name__}() took {total_time:.4f} seconds")
        return result
...
def get_image_from_string(
    ...
    if image_class is None and "type" in query_params:
        ...
        if image_type == "content":
            ...
            logger.warning(f"Unable to find image class for type='{image_type}' on '{url_string}' - Supported types are: content")
...
class BaseNoteCache(abc.ABC):
            ...
            with pelican_open(self.filename) as text:
                for line_number, line in enumerate(text.splitlines()):
                    ...
                    try:
                        ...
                        logger.warning(f"Parser error in file {self.filename} on line {line_number + 1}: {e}")

        except Exception:
            logger.exception(f"There was an error while processing the {self.filename}. ")
    ...
    def from_cache(cls, source_image: "SourceImage"):
        filename = os.path.join(os.path.dirname(source_image.filename), cls.note_filename())
...
class BaseNoteKeyValueCache(BaseNoteCache):
    def _parse_line(self, line: str) -> Tuple[str, str]:
        ...
        raise BaseNoteParseError("Wrong format. Expecting '<filename>: <value>' per line")
...
class BaseImage:
    ...
    def __init__(
        ...
        self.src_filename = os.path.join(os.path.expanduser(pelican_settings["PHOTO_LIBRARY"]), self.filename)
        ...
        if self.profile is None:
            ...
            try:
                ...
                logger.error(f"Unable to find profile '{profile_name}' for image '{filename}'")
                raise
...
class ArticleImage(BaseImage):
    ...
    def __init__(
        ...
        if self.filename_type == "filename":
            self.src_filename = os.path.join(g_generator.path, content.relative_dir, self.filename)
        ...
        if not os.path.isfile(self.src_filename):
            raise FileNotFound(f"No photo for {content.source_path} at {self.filename} source {self.src_filename}")

        photo = os.path.splitext(self.filename)[0].lower() + self.profile.article_file_suffix + self.profile.file_suffix
        thumb = os.path.splitext(self.filename)[0].lower() + self.profile.thumb_file_suffix + self.profile.file_suffix
        img = Image(
...
class ContentImage(BaseImage):
    def __init__(
        ...
        img = Image(
            ...
            dst=os.path.join(
                "photos",
                os.path.splitext(self.filename)[0].lower() + self.profile.article_file_suffix + self.profile.file_suffix,
            ),
            specs=self.profile.article_image_spec,
        )
        self.image = enqueue_image(img)
...
class ContentImageLightbox(BaseImage):
    def __init__(
        ...
        img = Image(
            ...
            dst=os.path.join(
                "photos",
                os.path.splitext(filename)[0].lower() + self.profile.gallery_file_suffix + self.profile.file_suffix,
            ),
        ...
        img = Image(
            ...
            dst=os.path.join(
                "photos",
                os.path.splitext(filename)[0].lower() + self.profile.thumb_file_suffix + self.profile.file_suffix,
            ),
            specs=self.profile.thumb_image_spec,
            is_thumb=True,
        )
...
class Gallery:
    ...
    def __init__(  # noqa: PLR0912 -- too many branches
        ...
        if not os.path.isdir(dir_gallery):
            raise GalleryNotFound("Gallery does not exist: {} at {}".format(location_parsed["location"], dir_gallery))
...
class GalleryImage(BaseImage):
    ...
    def __init__(
        ...
        if self.filename_type == "filename":
            self.src_filename = os.path.join(g_generator.path, self._gallery.content.relative_dir, self.filename)

        img = Image(
            src=self.src_filename,
            dst=os.path.join(
                "photos",
                os.path.splitext(self.filename)[0].lower() + self.profile.gallery_file_suffix + self.profile.file_suffix,
            ),
            specs=self.profile.gallery_image_spec,
        )
        if img.is_excluded:
        ...
        img = Image(
            ...
            dst=os.path.join(
                "photos",
                os.path.splitext(self.filename)[0].lower() + self.profile.thumb_file_suffix + self.profile.file_suffix,
            ),
            specs=self.profile.thumb_image_spec,
            is_thumb=True,
        )
...
class Image:
    ...
    def __init__(  # noqa: PLR0912, PLR0913, PLR0915 -- this function does a _lot_
        ...
        image_options: Dict[str, Any] = self._pelican_settings["PHOTO_DEFAULT_IMAGE_OPTIONS"].get(self.type)
        if image_options is None:
            image_options = {}
        if not isinstance(image_options, dict):
            logger.warning(f"photos: Wrong type for default image options for type {spec['type']}")
            image_options = {}
        ...
        self.output_filename = "{filename}.{extension}".format(
            ...
            extension=self._pelican_settings["PHOTO_FILE_EXTENSIONS"].get(self.type, self.type),
        )
        ...
        if isinstance(skip_operations, (list, tuple)):
            for item in self.pre_operations:
                if (isinstance(item, str) and item in skip_operations) or (isinstance(item, (list, tuple)) and item[0] in skip_operations):
        ...
        for operation in self.operations:
            ...
            if operation_name in ("main.resize", "ops.fit") and "size" not in operation_args:
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(source_image={self.source_image}, dst={self.dst} is_thumb={self.is_thumb})"
    ...
    def is_alpha(img: PILImage.Image) -> bool:
        return bool(img.mode in ("RGBA", "LA") or (img.mode == "P" and "transparency" in img.info))
    ...
    def process(  # noqa: PLR0912, PLR0915 -- this function does a lot
        ...
        logger.info(f"photos: make photo(PID: {process.pid}) {self.source_image.filename} -> {self.output_filename}")

        if os.path.isfile(self.output_filename) and os.path.getmtime(self.source_image.filename) <= os.path.getmtime(self.output_filename):
            logger.debug(f"Skipping orig: {os.path.getmtime(self.source_image.filename)} {os.path.getmtime(self.output_filename)}")
        ...
        if ispiexif:
            if self._pelican_settings["PHOTO_EXIF_KEEP"] and "exif" in image.info:
                ...
                try:
                    ...
                    logger.warning(f"There was an error reading exif data from '{self.source_image.filename}': {e}")
            else:
                self.exif_result = {}
        else:
        ...
        for i, operation in enumerate(operations):
            ...
            if func:
                ...
                logger.warning(f"Unable to find operation: '{operation_name}' for destination image '{self.dst}'")
    ...
    def _operation_manipulate_exif(self, image: PILImage.Image, image_meta: "Image") -> PILImage.Image:
        ...
        if self._pelican_settings["PHOTO_EXIF_COPYRIGHT"]:
            ...
            if not image_meta.exif_result["0th"].get(piexif.ImageIFD.Copyright):
                license = build_license(self._pelican_settings["PHOTO_EXIF_COPYRIGHT"], author)
    ...
    def _operation_remove_alpha(self, image: PILImage.Image) -> PILImage.Image:
        ...
        background = PILImage.new("RGB", image.size, self._pelican_settings["PHOTO_ALPHA_BACKGROUND_COLOR"])
    ...
    def _operation_watermark(self, image: PILImage.Image) -> PILImage.Image:
        ...
        if self._pelican_settings["PHOTO_WATERMARK_TEXT"]:
            ...
            default_font = os.path.join(os.path.dirname(os.path.realpath(__file__)), font_name)
            font_size = watermark_layer.size[0] // text_reducer
            if font_size < 1:
                logger.info(f"photos: font size {font_size} is to small skipping watermark")
                return image

            font = ImageFont.FreeTypeFont(font=default_font, size=font_size)
            text_size[0] = draw_watermark.textlength(self._pelican_settings["PHOTO_WATERMARK_TEXT"], font, direction="rtl")
        ...
        if self._pelican_settings["PHOTO_WATERMARK_IMG"]:
            ...
            mark_image_size = self._pelican_settings["PHOTO_WATERMARK_IMG_SIZE"] if self._pelican_settings["PHOTO_WATERMARK_IMG_SIZE"] else mark_image_size
...
class SourceImage:
    ...
    def __init__(self, filename):
        ...
        if file_type != "image":
            raise FileExcluded(f"Skipe file '{self.filename}' because MIME type is '{file_type}' but must be not 'image'.")
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(filename={self.filename}, mimetype={self.mimetype}, type={self.type})"
...
def get_process_pool():
    ...
    if g_process_pool is not None:
        logger.debug(f"Process pool in state {g_process_pool._state} and with {len(g_process_pool._pool)} worker(s) already exists")
...
def initialized(pelican: Pelican):  # noqa: PLR0915
    ...
    if pelican:
        ...
        pelican.settings.setdefault("PHOTO_FILE_EXTENSIONS", {"jpeg": "jpg", "webp": "webp"})
        ...
        pelican.settings.setdefault("PHOTO_INLINE_GALLERY_PATTERN", r"gallery::(?P<gallery_name>[/{}\w_-]+)")
        ...
        pelican.settings.setdefault("PHOTO_DEFAULT_IMAGE_OPTIONS", {"jpeg": {"optimize": True}})
...
def enqueue_image(img: Image) -> Image:
    ...
    elif g_image_cache[img.dst].source_image != img.source_image or g_image_cache[img.dst].spec != img.spec:
        raise InternalError(
...
def build_license(license, author):
    ...
    license_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), "licenses.json")
...
def detect_inline_images(content: pelican.contents.Content):  # noqa: PLR0912
    ...
    inline_images = {}
    if not content._content or not ("{photo}" in content._content or "{lightbox}" in content._content):
...
def detect_inline_galleries(content: Union[Article, Page]):
    ...
    if pelican_settings["PHOTO_INLINE_GALLERY_ENABLED"]:
        gallery_strings = re.finditer(pelican_settings["PHOTO_INLINE_GALLERY_PATTERN"], content._content)
        for m in gallery_strings:
            inline_galleries[str(m.group())] = process_content_galleries(content, m.group("gallery_name"))
...
def detect_inline_contents(content: Union[Article, Page]):
    ...
    inline_contents = {}
    content_strings = re.finditer(pelican_settings["PHOTO_INLINE_PATTERN"], content._content)
...
def detect_meta_images(content: pelican.contents.Content):
    ...
    for meta_name, meta_value in content.metadata.items():
        ...
        if meta_image_name and meta_prefix.lower().strip() == "image":
            images[meta_image_name.strip()] = get_image_from_string(url_string=meta_value, default_image_class=ContentImage)
...
def replace_inline_contents(content, inline_contents):
    for content_string, content_info in inline_contents.items():
        ...
        if content_info.type == "gallery":
            template_values["default_template_name"] = pelican_settings["PHOTO_INLINE_GALLERY_TEMPLATE"]
            template_values["galleries"] = image
            # We use the profile from the first gallery
            if len(image) > 0:
                profile = image[0].profile
            else:
                logger.warning(f"No gallery found for '{content_string}' in '{content.filename}'")
        elif content_info.type == "image":
            template_values["default_template_name"] = pelican_settings["PHOTO_INLINE_IMAGE_TEMPLATE"]
            template_values["image"] = image
            profile = image.profile
        elif content_info.type == "lightbox":
            template_values["default_template_name"] = pelican_settings["PHOTO_INLINE_LIGHTBOX_TEMPLATE"]
        ...
        if profile:
            content._content = content._content.replace(content_string, profile.render_template(**template_values))
        else:
            logger.warning(f"Unable to find profile for '{content_string}' in '{content.filename}'. Removing from output. Have a look for other errors or warnings to fix this issue.")
...
def replace_inline_galleries(content, inline_galleries):
    for gallery_string, galleries in inline_galleries.items():
        template = g_generator.get_template(pelican_settings["PHOTO_INLINE_GALLERY_TEMPLATE"])
        template_values = {
            "galleries": galleries,
            "SITEURL": pelican_settings["SITEURL"],
        }
        ...
        content._content = content._content.replace(gallery_string, template.render(**template_values))
...
def replace_inline_images(content, inline_images):
    for image_string, image_info in inline_images.items():
        ...
        if profile.has_template:
            ...
            lightbox_attr_list.append('{}="{}"'.format(pelican_settings["PHOTO_LIGHTBOX_GALLERY_ATTR"], gallery_name))
