Title: Sagrotan
Slug: sagrotan
Name: Sagrotan
Date: 2025-10-06 12:04
Location: Montreal / Canada
Category: Atari ST, Antivirus
Lang: en
Author: shazz
status: hidden
summary: This article is about Sagrotan...
image: {filename}../../../gallery/antiviruses/sagrotan.png
Tags: Antivirus

# Sagrotan 4.17

## Basic Information
* Version 4.17 released 1990/05/13
* Author: Henrik Alt
* Program language: German
* Can detect 15 viruses, 91 regular boot sectors, 106 total

# Main Part
The antivirus was written by German programmer Henrik Alt on 1990/05/13 and can recognize 106 boot sectors. When launching the program, you are greeted with a welcome screen containing information about the author and the program (photo 0). There are higher versions available, but they are translations and don't offer significant additional benefits. The main menu contains several tabs. Let's go through them first!

### Tabs:
1. _Desk_
2. _Virus_
3. _Laufwerk_
4. _Bibliothek_
5. _Allerhei_
6. _Hilfe_

## Desk
This tab contains:

* **SAGROTAN**
* **UMLAUT**
* **Schreibschutz**
* **Multi-ACC**

1. This tab displays the information seen when first starting the program
2. Print settings for EPSON printers
3. Enable software write protection for the selected drive
4. More detailed drive settings (photo 1)

## Virus
This tab contains:

* **Bootsektor prüfen**
* **Bootsektor schützen**
* **Bootsektor löschen**
* **Bootsektor reparieren**
* **Analyse wiederholen**
* **Disk Info**
* **Speichertest (Memory)**
* **Dateien (Files)**
* **Optionen**
* **Komplettprüfung**
* **Ende**

1. Allows checking the boot sector for viruses and shows whether it is executable or not (photo 2)
2. Allows writing the Sagrotan vaccine to the boot sector
3. Allows complete cleaning of the boot sector
4. Allows restoring the boot sector by writing a boot sector of your choice to it (photo 3)
5. Allows repeating the boot sector analysis
6. Shows disk information
7. Allows checking memory for viruses
8. Allows checking a file for Link Viruses
9. Allows configuring the scan for Link Viruses (photo 4)
10. Allows checking all programs on the disk for Link Viruses
11. Allows exiting Sagrotan

## Laufwerk
This tab allows you to select the drive (Drive A, Drive B... and hard drive) on which all operations will be performed.

## Bibliothek
This tab contains:

* **Alle Bootsektoren speichern**
* **Bootsektor speichern**
* **Virus speichern**
* **Programinformationen speichern**
* **Zeige Vergleichsdateien**

1. Allows saving all boot sectors
2. Allows saving a boot sector
3. Allows saving a virus
4. Allows saving program information
5. Shows everything that Sagrotan can recognize

## Allerlei
This tab contains:

* **Einstellung sichern**
* **Druckerprotokoll**
* **Blitter**
* **Textgröße 13**
* **Textgröße 6**
* **Kaltstart**

1. Allows saving settings
2. Printer log
3. Blitter
4. Allows setting the text size to 13
5. Allows setting the text size to 6
6. Allows performing a cold boot

## Hilfe

* **Extrahilfe**
* **Warnmeldungen**

1. Extra help
2. Warnings